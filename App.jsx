...
export default function App() {
  ...
  function reloadFromSupabase() {
    return loadAllParticipants().then(function(fresh) {
      if (fresh && fresh.length > 0) {
        // Merge: for each fresh participant, use the fresh version.
        // This way a reload never loses in-memory changes for participants
        // not yet persisted (though there should be none with updateOnePart).
        setParts(fresh);
      }
    }).catch(function(e) { console.error("reloadFromSupabase error:", e); });
  }
  ...
  // Safe single-participant update: merges into current state without stomping others.
  // Use this instead of saveParts() when only one participant changed.
  function updateOnePart(updatedParticipant) {
    saveParticipant(updatedParticipant);
    setParts(function(prev) {
      var found = false;
      var next = prev.map(function(p) {
        if (p.id === updatedParticipant.id) { found = true; return updatedParticipant; }
        return p;
      });
      if (!found) next = next.concat([updatedParticipant]);
      return next;
    });
  }
  ...
  var sh = {view:view,setView:setView,parts:parts,scored:scored,results:results,settings:settings,currentUser:currentUser,setCurrentUser:setCurrentUser,adminUnlocked:adminUnlocked,setAdminUnlocked:setAdminUnlocked,saveParts:saveParts,updateOnePart:updateOnePart,saveResults:saveResults,saveSettings:saveSettings,reloadFromSupabase:reloadFromSupabase};
...
function PredictView(props) {
  ...
  function submit() {
    // Find only this user's current data — never touch other participants
    var base = null;
    for (var i = 0; i < props.parts.length; i++) {
    ...
    // updateOnePart: saves to Supabase AND merges into state via functional update
    // so it never overwrites other participants' statuses
    props.updateOnePart(target);
...
function AdminView(props) {
  ...
  // Reload fresh data from Supabase every time the approvals tab is opened
  // This ensures the admin always sees the latest statuses
  useEffect(function() {
    if (tab === "approvals" && props.reloadFromSupabase) {
      props.reloadFromSupabase();
    }
  }, [tab]);

  // Also reload once when AdminView first mounts
  useEffect(function() {
    if (props.reloadFromSupabase) props.reloadFromSupabase();
  }, []);
  ...
  function removeP(id) {
    ...
    deleteParticipant(id).then(function(r) {
      if (!r || !r.ok) { flash("❌ Error al eliminar. Intenta de nuevo."); return; }
