...
export default function App() {
  ...
  // ── THE ONLY WAY TO UPDATE STATE: write to Supabase, then reload from Supabase ──
  // This eliminates ALL race conditions and stale-state bugs.
  function reloadParts() {
    return loadAllParticipants().then(function(fresh) {
      setParts(fresh && fresh.length > 0 ? fresh : []);
    }).catch(function(e) { console.error("reloadParts error:", e); });
  }

  // Write one participant to Supabase, then reload ALL participants into state.
  // This is the ONLY mutation path — no optimistic updates, no partial merges.
  ...
  // Delete one participant from Supabase, then reload.
...
function AdminView(props) {
  ...
  // Reload from Supabase when AdminView mounts so admin sees latest data
  useEffect(function() {
    if (props.reloadParts) props.reloadParts();
  }, []);
