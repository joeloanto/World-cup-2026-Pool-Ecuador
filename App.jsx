...
// ─── DATES ────────────────────────────────────────────────────────────────────
var TOURNAMENT_START = new Date("2026-06-11T23:00:00Z");
...
var R32_SLOTS = [
  {a:{type:"3rd",gs:"A"},     b:{type:"2nd",g:"B"},  label:"M73"},
  {a:{type:"1st",g:"E"},      b:{type:"3rd",gs:"D"},  label:"M74"},
  {a:{type:"1st",g:"C"},      b:{type:"2nd",g:"F"},   label:"M75"},
  {a:{type:"1st",g:"F"},      b:{type:"2nd",g:"C"},   label:"M76"},
  {a:{type:"1st",g:"I"},      b:{type:"3rd",gs:"F"},  label:"M77"},
  {a:{type:"2nd",g:"E"},      b:{type:"2nd",g:"I"},   label:"M78"},
  {a:{type:"1st",g:"A"},      b:{type:"3rd",gs:"E"},  label:"M79"},
  {a:{type:"1st",g:"L"},      b:{type:"3rd",gs:"K"},  label:"M80"},
  {a:{type:"1st",g:"D"},      b:{type:"3rd",gs:"B"},  label:"M81"},
  {a:{type:"1st",g:"G"},      b:{type:"3rd",gs:"I"},  label:"M82"},
  {a:{type:"1st",g:"K"},      b:{type:"2nd",g:"L"},   label:"M83"},
  {a:{type:"1st",g:"H"},      b:{type:"2nd",g:"J"},   label:"M84"},
  {a:{type:"1st",g:"B"},      b:{type:"3rd",gs:"J"},  label:"M85"},
  {a:{type:"1st",g:"J"},      b:{type:"2nd",g:"H"},   label:"M86"},
  {a:{type:"2nd",g:"K"},      b:{type:"3rd",gs:"L"},  label:"M87"},
  {a:{type:"2nd",g:"D"},      b:{type:"3rd",gs:"G"},  label:"M88"}
];
...
function storeSet(k, v) {
  ...
  return sbFetch("/rest/v1/kvstore", {
    ...
    if (r && !r.ok) r.text().then(function(t) { console.error("storeSet error:", t); });
  }).catch(function(e) { console.error("storeSet error:", e); });
}

// Save one participant to their own row atomically
function saveParticipant(participant) {
  return storeSet(KP + ":" + participant.id, JSON.stringify(participant));
}
...
function deleteParticipant(participantId) {
  return sbFetch("/rest/v1/kvstore?key=eq." + encodeURIComponent(KP + ":" + participantId), {
    method: "DELETE"
  }).catch(function(e) { console.error("deleteParticipant error:", e); });
}
...
function resolveSlot(slot, gRes, best8, used) {
  ...
  if (slot.type === "3rd") {
    ...
    // First try best8 (only populated when all groups complete)
    ...
    // Fallback: show current 3rd of the specific group as preview
    if (parts.length === 1 && gRes[parts[0]] && gRes[parts[0]].third) {
      return gRes[parts[0]].third;
    }
    return parts.length === 1 ? ("3ro Grp " + parts[0]) : "Mejor 3ro";
  }
  return "TBD";
}
...
function calcScore(pred, results, penalty, isLate) {
  ...
  // 2 & 3. Group qualifiers: award 3pts if predicted team advances by ANY means
  // (1st, 2nd, or best 3rd) — criteria: did the team actually qualify from groups?
  var pQ = computeQualifiers(pSc);
  var rQ = computeQualifiers(rSc);
  var allGroupsDone = allGroupsComplete(rSc);

  // Build set of all teams that actually advanced (1st + 2nd from each group + best8 3rds)
  var realAdvanced = {};
  for (var gi = 0; gi < gkeys.length; gi++) {
    var g = gkeys[gi];
    if (!isGroupComplete(g, rSc)) continue;
    if (rQ.gRes[g].winner)   realAdvanced[rQ.gRes[g].winner]   = true;
    if (rQ.gRes[g].runnerUp) realAdvanced[rQ.gRes[g].runnerUp] = true;
  }
  if (allGroupsDone) {
    for (var b8i = 0; b8i < rQ.best8.length; b8i++) realAdvanced[rQ.best8[b8i]] = true;
  }

  // Check predicted 1st/2nd from each group
  for (var gi2 = 0; gi2 < gkeys.length; gi2++) {
    var g2 = gkeys[gi2];
    if (!isGroupComplete(g2, rSc)) continue;
    var pTeams = [pQ.gRes[g2].winner, pQ.gRes[g2].runnerUp];
    for (var ti = 0; ti < pTeams.length; ti++) {
      var t = pTeams[ti];
      if (t && realAdvanced[t]) pts += GQ_PTS;
    }
  }

  // Check predicted best-3rd picks (only scoreable when all groups done)
  if (allGroupsDone) {
    for (var bi = 0; bi < pQ.best8.length; bi++) {
      var bt = pQ.best8[bi];
      if (bt && realAdvanced[bt]) pts += GQ_PTS;
    }
  }
...
function getBreakdown(pred, results, penalty, isLate) {
  ...
  // Build set of all teams that actually advanced
  var realAdv = {};
  for (var gai = 0; gai < gkeys.length; gai++) {
    var ga = gkeys[gai];
    if (!isGroupComplete(ga, rSc)) continue;
    if (rQ.gRes[ga].winner)   realAdv[rQ.gRes[ga].winner]   = true;
    if (rQ.gRes[ga].runnerUp) realAdv[rQ.gRes[ga].runnerUp] = true;
  }
  if (allDone) {
    for (var rb8 = 0; rb8 < rQ.best8.length; rb8++) realAdv[rQ.best8[rb8]] = true;
  }
  ...
  for (var gi2 = 0; gi2 < gkeys.length; gi2++) {
    ...
    for (var qi = 0; qi < qTeams.length; qi++) {
      var qt = qTeams[qi];
      if (!qt) continue;
      // Hit if team actually advanced by ANY means (1st, 2nd, or best 3rd)
      var qhit = groupDone2 && !!realAdv[qt];
      // If group done but best8 not yet (not all groups done), only check 1st/2nd
      if (groupDone2 && !allDone) {
        qhit = (qt === rQ.gRes[g2].winner || qt === rQ.gRes[g2].runnerUp);
      }
...
export default function App() {
  ...
  function saveParts(list) {
    // Only update local React state — Supabase writes happen individually
    // via saveParticipant() to avoid overwriting other users' data
    setParts(list);
  }

  // Use this when you need to both update state AND save one participant to Supabase
  function saveOnePart(updatedParticipant, allParts) {
...
function PredictView(props) {
  ...
  function setScore(g, i, side, val) {
    ...
    setPred(function(prev) {
      ...
      // auto-save ONLY this user's row — never touch other participants
      if (props.currentUser) {
        var currentPart = null;
        for (var pi = 0; pi < props.parts.length; pi++) {
          if (props.parts[pi].id === props.currentUser.id) {
            currentPart = Object.assign({}, props.parts[pi], {prediction: next, lastEditedAt: Date.now()});
            break;
          }
        }
        if (currentPart) saveParticipant(currentPart);
      }
      return next;
    });
  }
  ...
  function setWinner(round, idx, team) {
    ...
    setPred(function(prev) {
      ...
      // auto-save ONLY this user's row
      if (props.currentUser) {
        var currentPart2 = null;
        for (var pi2 = 0; pi2 < props.parts.length; pi2++) {
          if (props.parts[pi2].id === props.currentUser.id) {
            currentPart2 = Object.assign({}, props.parts[pi2], {prediction: next, lastEditedAt: Date.now()});
            break;
          }
        }
        if (currentPart2) saveParticipant(currentPart2);
      }
      return next;
    });
  }
  ...
  function submit() {
    ...
    // Only save this user's data to Supabase
    if (target) saveParticipant(target);
...
function AdminView(props) {
  ...
  function approve(id) {
    var target = null;
    var updated = props.parts.map(function(p) {
      if (p.id === id) { target = Object.assign({}, p, {status:"approved"}); return target; }
      return p;
    });
    // Save ONLY this participant to Supabase — don't touch others
    if (target) saveParticipant(target);
    // Update local state only (no Supabase write for others)
    props.saveParts(updated);
    flash("¡Aprobado!");
  }
  ...
  function rejectP(id) {
    var target = null;
    var updated = props.parts.map(function(p) {
      if (p.id === id) { target = Object.assign({}, p, {status:"rejected"}); return target; }
      return p;
    });
    if (target) saveParticipant(target);
    props.saveParts(updated);
  ...
  function resetF(id) {
    ...
    var updated = props.parts.map(function(p) {
      if (p.id !== id) return p;
      target = Object.assign({}, p, {prediction:{groupScores:{}},status:"draft",submittedAt:null,lastEditedAt:null});
      return target;
    });
    if (target) saveParticipant(target);
    props.saveParts(updated);
        ...
        <div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:20}}>
            {(function() {
              ...
              bt2(0, {});
              // Fallback: if best8 not determined yet, show current 3rd of that group as preview
              function resolveThird(gs, idx, side) {
                // First try the backtracking assignment (used when all groups done)
                if (asgn2[idx+"_"+side]) return asgn2[idx+"_"+side];
                // Fallback: show current 3rd-place team from that specific group
                var grp = gs.split("/")[0]; // single group now
                return (aQ.gRes[grp] && aQ.gRes[grp].third) || ("3ro Grp " + grp);
              }
              return R32_SLOTS.map(function(slot, i) {
                var tA = slot.a.type==="3rd" ? resolveThird(slot.a.gs, i, "a") : resolveSlot(slot.a, aQ.gRes, aQ.best8, {});
                var tB = slot.b.type==="3rd" ? resolveThird(slot.b.gs, i, "b") : resolveSlot(slot.b, aQ.gRes, aQ.best8, {});
