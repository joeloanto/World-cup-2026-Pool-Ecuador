...
function saveParticipant(participant) {
  // Pass object directly — storeSet handles stringify internally
...
function AdminView(props) {
  ...
  function approve(id) {
    ...
    saveParticipant(target).then(function(result) {
      if (!result) {
        // Revert on failure
        props.saveParts(props.parts);
        flash("❌ Error al aprobar. Intenta de nuevo.");
      } else {
        flash("¡Aprobado!");
      }
    });
  ...
  function rejectP(id) {
    ...
    saveParticipant(target).then(function(result) {
      if (!result) {
        props.saveParts(props.parts);
        flash("❌ Error al rechazar. Intenta de nuevo.");
      }
    });
  }
  ...
  function removeP(id) {
    ...
    // Optimistic removal
    var prevParts = props.parts.slice();
    props.saveParts(props.parts.filter(function(p) { return p.id !== id; }));
    deleteParticipant(id).then(function(result) {
      if (!result || !result.ok) {
        // Revert if delete failed
        props.saveParts(prevParts);
        flash("❌ Error al eliminar. Intenta de nuevo.");
      } else {
  ...
  function resetF(id) {
    ...
    saveParticipant(target).then(function(result) {
      if (!result) {
        props.saveParts(props.parts);
        flash("❌ Error al borrar pronóstico. Intenta de nuevo.");
      } else {
