...
// ─── DATES ────────────────────────────────────────────────────────────────────
var TOURNAMENT_START = new Date("2026-06-12T03:45:00Z");
var LATE_DEADLINE    = new Date("2026-07-20T04:00:00Z");
var EDIT_DEADLINE    = new Date("2026-07-20T04:00:00Z");
...
// ─── SUPABASE STORAGE ────────────────────────────────────────────────────────
// DEFINITIVE ARCHITECTURE: each participant has their own Supabase row.
// Key format: KP + ":" + participant.id  (e.g. "ecuador-v1p:abc123")
// This completely eliminates race conditions — no shared arrays.
...
function sbFetch(path, opts) {
  ...
  return fetch(SB_URL + path, Object.assign({}, opts, { headers: headers }))
    .catch(function(e) { console.error("Supabase fetch error:", e); return null; });
}

// Read a single key
function storeGet(k) {
  return sbFetch("/rest/v1/kvstore?select=value&key=eq." + encodeURIComponent(k), {
...
function storeSet(k, v) {
  var json = typeof v === "string" ? v : JSON.stringify(v);
  return sbFetch("/rest/v1/kvstore", {
    method: "POST",
    headers: { "Prefer": "resolution=merge-duplicates,return=representation" },
    body: JSON.stringify({ key: k, value: json, updated_at: new Date().toISOString() })
  }).then(function(r) {
    if (!r) { console.error("storeSet: no response for key", k); return null; }
    if (!r.ok) {
      return r.text().then(function(t) { console.error("storeSet error for key", k, ":", t); return null; });
    }
    return r.json().catch(function() { return null; });
  }).catch(function(e) { console.error("storeSet error:", e); return null; });
}

// Save one participant to their own row atomically
function saveParticipant(participant) {
  return storeSet(KP + ":" + participant.id, participant);
}

// Delete one participant row
function deleteParticipant(participantId) {
  return sbFetch("/rest/v1/kvstore?key=eq." + encodeURIComponent(KP + ":" + participantId), {
    method: "DELETE",
    headers: { "Prefer": "return=minimal" }
  }).then(function(r) {
    if (!r || !r.ok) { console.error("deleteParticipant failed for id", participantId); }
    return r;
  }).catch(function(e) { console.error("deleteParticipant error:", e); return null; });
}

// Load ALL participants for this pool (reads all rows with prefix KP + ":")
function loadAllParticipants() {
  return sbFetch("/rest/v1/kvstore?select=key,value&key=like." + KP + ":%25" + "&order=updated_at.asc", {
    headers: { "Accept": "application/json" }
  }).then(function(r) {
    if (!r || !r.ok) return [];
    return r.json();
    ...
    for (var i = 0; i < rows.length; i++) {
      try {
        var p = JSON.parse(rows[i].value);
        if (p && p.id) parts.push(p);
      } catch(e) {}
    }
    return parts;
...
function calcStandings(g, scores) {
  ...
  // FIFA tiebreaker: head-to-head record among tied teams
  function getH2H(tiedTeams) {
    var h2h = {};
    for (var i = 0; i < tiedTeams.length; i++) {
      h2h[tiedTeams[i]] = {pts:0, gd:0, gf:0};
    }
    for (var fi2 = 0; fi2 < fixtures.length; fi2++) {
      var fx2 = fixtures[fi2];
      var sc2 = (scores || {})[g + "_" + fi2];
      if (!sc2 || sc2.h === "" || sc2.a === "" || sc2.h === undefined || sc2.a === undefined) continue;
      var h2 = fx2[0]; var a2 = fx2[1];
      // Only count if BOTH teams are in the tied group
      if (tiedTeams.indexOf(h2) < 0 || tiedTeams.indexOf(a2) < 0) continue;
      var hg2 = parseInt(sc2.h) || 0;
      var ag2 = parseInt(sc2.a) || 0;
      h2h[h2].gf += hg2; h2h[h2].gd += hg2 - ag2;
      h2h[a2].gf += ag2; h2h[a2].gd += ag2 - hg2;
      if (hg2 > ag2) { h2h[h2].pts += 3; }
      else if (hg2 === ag2) { h2h[h2].pts += 1; h2h[a2].pts += 1; }
      else { h2h[a2].pts += 3; }
    }
    return h2h;
  }
  ...
  // 4. H2H goals for among tied teams
  // 5. Overall goal difference
  // 6. Overall goals for
  // 7. Alphabetical (last resort)
  var sorted = teams.slice().sort(function(a, b) {
    if (st[b].pts !== st[a].pts) return st[b].pts - st[a].pts;
    // Find all teams tied on points with both a and b
    var tiedPts = st[a].pts;
    var tiedGroup = teams.filter(function(t) { return st[t].pts === tiedPts; });
    if (tiedGroup.length > 1) {
      var h2h = getH2H(tiedGroup);
...
function calcScore(pred, results, penalty, isLate) {
  ...
  // 2. Group qualifiers: award points if predicted team advances by ANY means
  // (1st, 2nd, or best 3rd) — 3 pts regardless of which position they finished.
  // Requires all 72 group matches to be complete so best-3rd is determined.
  var pQ = computeQualifiers(pSc);
  var rQ = computeQualifiers(rSc);
  var allDoneForQ = allGroupsComplete(rSc);
  for (var gi = 0; gi < gkeys.length; gi++) {
    var g = gkeys[gi];
    if (!isGroupComplete(g, rSc)) continue; // group not finished yet
    // Teams the user predicted to advance from this group (1st and 2nd)
    var pTeams = [pQ.gRes[g].winner, pQ.gRes[g].runnerUp];
    // Teams that actually advance from this group: 1st, 2nd, and (if all groups done) best 3rd
    var rAdvance = [rQ.gRes[g].winner, rQ.gRes[g].runnerUp];
    if (allDoneForQ && rQ.best8.indexOf(rQ.gRes[g].third) >= 0) {
      rAdvance.push(rQ.gRes[g].third);
    }
    for (var ti = 0; ti < pTeams.length; ti++) {
      var t = pTeams[ti];
      if (t && rAdvance.indexOf(t) >= 0) pts += GQ_PTS;
    }
  }

  // 3. Best 3rd place predictions: award points if that team advances by ANY means
  if (allDoneForQ) {
    for (var bi = 0; bi < pQ.best8.length; bi++) {
      var bt = pQ.best8[bi];
      // Find which group this team is from
      var btGroup = null;
      var gk2 = Object.keys(GROUPS);
      for (var gi2 = 0; gi2 < gk2.length; gi2++) {
        if (rQ.gRes[gk2[gi2]].winner === bt || rQ.gRes[gk2[gi2]].runnerUp === bt || rQ.gRes[gk2[gi2]].third === bt) {
          btGroup = gk2[gi2]; break;
        }
      }
      // Award points if team advanced by any means (1st, 2nd, or best 3rd)
      var rAdvBt = btGroup ? [rQ.gRes[btGroup].winner, rQ.gRes[btGroup].runnerUp] : [];
      if (rQ.best8.indexOf(bt) >= 0) rAdvBt.push(bt);
      if (rAdvBt.indexOf(bt) >= 0) pts += GQ_PTS;
    }
  }
...
function getBreakdown(pred, results, penalty, isLate) {
  ...
  for (var gi2 = 0; gi2 < gkeys.length; gi2++) {
    ...
    // Build list of teams that actually advanced from this group
    var rAdv2 = groupDone2 ? [rQ.gRes[g2].winner, rQ.gRes[g2].runnerUp] : [];
    if (allDone && groupDone2 && rQ.best8.indexOf(rQ.gRes[g2].third) >= 0) rAdv2.push(rQ.gRes[g2].third);
    for (var qi = 0; qi < qTeams.length; qi++) {
      var qt = qTeams[qi];
      if (!qt) continue;
      var qhit = groupDone2 && rAdv2.indexOf(qt) >= 0;
  ...
  for (var b8i = 0; b8i < pQ.best8.length; b8i++) {
    var bt = pQ.best8[b8i];
    // Find group of this team
    var btGrp = null;
    var gk3 = Object.keys(GROUPS);
    for (var gi3 = 0; gi3 < gk3.length; gi3++) {
      if (GROUPS[gk3[gi3]].indexOf(bt) >= 0) { btGrp = gk3[gi3]; break; }
    }
    var rAdvBt2 = (allDone && btGrp) ? [rQ.gRes[btGrp].winner, rQ.gRes[btGrp].runnerUp] : [];
    if (allDone && rQ.best8.indexOf(bt) >= 0) rAdvBt2.push(bt);
    var bhit = allDone && rAdvBt2.indexOf(bt) >= 0;
...
export default function App() {
  ...
  // ── MUTATION API ────────────────────────────────────────────────────────────
  // Rule: write to Supabase for the ONE changed participant, then update ONLY
  // that single row in local state. Never reload ALL participants — Supabase may
  // have stale bulk-written rows from old code that would overwrite newer data.

  function reloadParts() {
    return loadAllParticipants().then(function(fresh) {
      if (fresh && fresh.length > 0) setParts(fresh);
    }).catch(function(e) { console.error("reloadParts error:", e); });
  }

  // Write participant to Supabase, then splice ONLY that row into local state.
  function persistPart(participant) {
    return saveParticipant(participant).then(function() {
      setParts(function(prev) {
        var found = false;
        var next = prev.map(function(p) {
          if (p.id === participant.id) { found = true; return participant; }
          return p;
        });
        return found ? next : next.concat([participant]);
      });
    });
  }
  ...
  function removePart(id) {
    return deleteParticipant(id).then(function() {
      setParts(function(prev) {
  ...
  useEffect(function() {
    setLoading(true);
    var timeout = setTimeout(function() { setLoading(false); }, 8000);
    Promise.all([
      loadAllParticipants(),
      storeGet(KR),
      storeGet(KS)
      ...
      if (vals[0] && vals[0].length > 0) setParts(vals[0]);
  ...
  var sh = {view:view,setView:setView,parts:parts,scored:scored,results:results,settings:settings,currentUser:currentUser,setCurrentUser:setCurrentUser,adminUnlocked:adminUnlocked,setAdminUnlocked:setAdminUnlocked,persistPart:persistPart,removePart:removePart,reloadParts:reloadParts,saveResults:saveResults,saveSettings:saveSettings};
    ...
    <div style={S.page}>
      ...
      <div style={{marginTop:"1.5rem",borderRadius:12,border:"0.5px solid var(--color-border-tertiary)",overflow:"hidden"}}>
        ...
        <div style={{padding:"8px 14px",borderTop:"0.5px solid var(--color-border-tertiary)",background:"var(--color-background-secondary)"}}>
          <div style={{fontSize:11,color:"var(--color-text-tertiary)"}}>Desempate: gana quien envió primero · Edición hasta 11 jun 6pm CT · Envío tardío hasta 14 jun 11pm CT</div>
        </div>
      </div>
    </div>
  );
...
function JoinView(props) {
  ...
  function go() {
    ...
    // Fire-and-forget save; user can start entering predictions immediately
    saveParticipant(user);
    // Add to local parts array without waiting — user needs to go to predict view now
    props.setCurrentUser(user);
    props.setView("predict");
  }
  ...
  return (
    <div style={S.page}>
      ...
      {pastDeadline ? (
        ...
        <div style={{...S.info,marginBottom:12}}>⏰ Sin penalización hasta el 11 jun 6pm CT. Edición permitida hasta esa fecha.</div>
      )}
      {!pastDeadline && (
        <div>
...
function PredictView(props) {
  ...
  function submit() {
    var base = null;
    for (var i = 0; i < props.parts.length; i++) {
      if (props.parts[i].id === props.currentUser.id) { base = props.parts[i]; break; }
    }
    if (!base) base = props.currentUser;
    var target = Object.assign({}, base, {
      prediction: pred,
      status: "pending",
      submittedAt: base.submittedAt || Date.now(),
      lastEditedAt: Date.now()
    });
    // persistPart: write to Supabase THEN reload all participants from Supabase.
    // This is the only safe way — no stale state can affect other participants.
    props.persistPart(target);
    ...
    <div style={{...S.page, maxWidth:660}}>
      ...
      {!pastDeadline && !editClosed && isLocked && !editing && (
        <div style={{...S.info,display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
          ...
          <div style={{fontSize:13}}>Predicciones enviadas. Haz clic en Editar para actualizar antes del 11 jun 6pm CT.</div>
        </div>
      )}
      {editing && (
...
function AdminView(props) {
  ...
  function approve(id) {
    var target = null;
    for (var i = 0; i < props.parts.length; i++) {
      if (props.parts[i].id === id) { target = Object.assign({}, props.parts[i], {status:"approved"}); break; }
    }
    if (!target) return;
    props.persistPart(target).then(function() { flash("¡Aprobado!"); });
  }

  function rejectP(id) {
    var target = null;
    for (var i = 0; i < props.parts.length; i++) {
      if (props.parts[i].id === id) { target = Object.assign({}, props.parts[i], {status:"rejected"}); break; }
    }
    if (!target) return;
    props.persistPart(target);
  }
  ...
  function removeP(id) {
    ...
    props.removePart(id).then(function() { flash("Participante eliminado."); });
  }
  ...
  function resetF(id) {
    ...
    var target = null;
    for (var i = 0; i < props.parts.length; i++) {
      if (props.parts[i].id === id) {
        target = Object.assign({}, props.parts[i], {prediction:{groupScores:{}},status:"draft",submittedAt:null,lastEditedAt:null});
        break;
      }
    }
    if (!target) return;
    props.persistPart(target).then(function() { flash("Pronóstico borrado."); });
  }
      ...
      {tab === "settings" && (
        <div style={S.card}>
          ...
          <div style={{marginTop:16,padding:"10px 12px",borderRadius:8,background:"var(--color-background-secondary)",fontSize:12,color:"var(--color-text-secondary)"}}>
            ...
            <div>🔒 Edición cierra: <strong>11 jun 2026, 6pm CT</strong></div>
