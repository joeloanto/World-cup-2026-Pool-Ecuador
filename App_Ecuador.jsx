...
var EDIT_DEADLINE    = new Date("2026-06-11T23:00:00Z");
...
function loadAllParticipants() {
  return sbFetch("/rest/v1/kvstore?select=key,value&key=like." + encodeURIComponent(KP + ":%25") + "&order=updated_at.asc", {
    headers: { "Accept": "application/json" }
  }).then(function(r) {
    if (!r || !r.ok) return [];
    return r.json();
...
export default function App() {
  ...
  function saveParts(list) {
    setParts(list);
    // Save each participant individually — no shared array, no race conditions
    for (var i = 0; i < list.length; i++) {
      saveParticipant(list[i]);
    }
  }
...
function AdminView(props) {
  ...
  function removeP(id) {
    ...
    var newList = props.parts.filter(function(p) { return p.id !== id; });
    props.saveParts(newList);
    deleteParticipant(id);
  }

  function resetF(id) {
    if (!window.confirm("¿Borrar el pronóstico? El participante tendrá que re-enviarlo.")) return;
    var newList = props.parts.map(function(p) {
      if (p.id !== id) return p;
