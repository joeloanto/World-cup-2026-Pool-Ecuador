...
// Save a single participant atomically - avoids race conditions
function storeSetParticipant(kp, participant) {
  var partKey = kp + ":" + participant.id;
  return supabaseSet(partKey, JSON.stringify(participant));
}

// Load all participants for a pool by scanning keys with prefix
function storeGetAllParticipants(kp) {
  return fetch(SB_URL + "/rest/v1/kvstore?select=value&key=like." + encodeURIComponent(kp + ":%25"), {
    headers: Object.assign({}, sbHeaders(), {"Accept": "application/json"})
  }).then(function(r) {
    if (!r.ok) throw new Error("HTTP " + r.status);
    return r.json();
    ...
    for (var i = 0; i < rows.length; i++) {
      try { parts.push(JSON.parse(rows[i].value)); } catch(e) {}
    }
...
export default function App() {
  ...
  useEffect(function() {
    ...
    // Try loading participants from individual keys first (avoids race conditions)
    // Fall back to the array key for backwards compatibility
    Promise.all([
      storeGetAllParticipants(KP).then(function(parts) {
        return parts || storeGet(KP);
      }),
      storeGet(KR),
      storeGet(KS)
    ]).then(function(vals) {
  ...
  function saveParts(list) {
    setParts(list);
    // Save full list for backwards compatibility
    storeSet(KP, list);
    // Also save each participant individually to avoid race conditions
    for (var i = 0; i < list.length; i++) {
      storeSetParticipant(KP, list[i]);
    }
  }
