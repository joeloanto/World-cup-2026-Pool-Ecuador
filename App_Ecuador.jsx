...
export default function App() {
  ...
  var scored = useMemo(function() {
    return parts.slice().map(function(p) {
      ...
      var base = calcScore(p.prediction, results, settings.penalty, p.isLate);
      var bonus = typeof p.bonusPoints === "number" && !isNaN(p.bonusPoints) ? p.bonusPoints : 0;
      copy.score = base + bonus;
      return copy;
    }).sort(function(a, b) {
...
function PredictView(props) {
  ...
  function buildR32Pairs() {
    var pairs = [];
    var used3rd = {};
    for (var i = 0; i < R32_SLOTS.length; i++) {
      var slot = R32_SLOTS[i];
      var ta = resolveSlot(slot.a, qualData.gRes, qualData.best8, used3rd);
      var tb = resolveSlot(slot.b, qualData.gRes, qualData.best8, used3rd);
      if (slot.a.type === "3rd" && ta && ta !== "TBD" && !ta.startsWith("3ro")) used3rd[ta] = true;
      if (slot.b.type === "3rd" && tb && tb !== "TBD" && !tb.startsWith("3ro")) used3rd[tb] = true;
      pairs.push({ a: ta, b: tb, label: slot.label });
    }
    return pairs;
  }
          ...
          {player && bd && (
            <div>
              <div style={{...S.card, marginBottom:16, display:"flex", alignItems:"center", gap:16}}>
                ...
                <div style={{textAlign:"center"}}>
                  <div style={{fontSize:36,fontWeight:500,lineHeight:1,color:"var(--color-text-warning)"}}>{bd.total + (typeof player.bonusPoints === "number" && !isNaN(player.bonusPoints) ? player.bonusPoints : 0)}</div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(110px,1fr))",gap:8,marginBottom:20}}>
                ...
                {(typeof player.bonusPoints === "number" && !isNaN(player.bonusPoints) && player.bonusPoints !== 0) && (
                  <div style={{...S.card, textAlign:"center", padding:"0.6rem 0.5rem"}}>
                    <div style={{fontSize:18}}>🎁</div>
                    <div style={{fontSize:11,color:"var(--color-text-secondary)",margin:"3px 0"}}>Bonus admin</div>
                    <div style={{fontSize:18,fontWeight:500,color:player.bonusPoints>0?"#27500A":"var(--color-text-danger)"}}>{player.bonusPoints > 0 ? "+" : ""}{player.bonusPoints}</div>
                  </div>
                )}
              </div>
              {sections.map(function(sec) {
                  ...
                  <div key={p.id} style={{borderBottom:"0.5px solid var(--color-border-tertiary)",padding:"8px 0"}}>
                    <div style={{display:"flex",alignItems:"center",gap:10}}>
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:6}}>
                        <span style={{fontSize:12,color:"var(--color-text-secondary)"}}>🎁 Bonus:</span>
                        <BonusInput
                          value={p.bonusPoints || 0}
                          onChange={function(val) {
                            props.saveParts(props.parts.map(function(x) {
                              return x.id === p.id ? Object.assign({}, x, {bonusPoints: val}) : x;
                            }));
                          }.bind(null, p)}
                        />
                        <span style={{fontSize:11,color:"var(--color-text-tertiary)"}}>pts</span>
                      </div>
                    ...
                    {(p.bonusPoints||0) !== 0 && (
                      <div style={{fontSize:11,color:"#27500A",marginTop:3,marginLeft:36}}>
                        🎁 {p.bonusPoints > 0 ? "+" : ""}{p.bonusPoints} pts bonus aplicados
                      </div>
                    )}
                  </div>
                );
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:20}}>
            {(function() {
              ...
              return R32_SLOTS.map(function(slot, i) {
                var tA = resolveSlot(slot.a, aQ.gRes, aQ.best8, used3rdAdmin);
                var tB = resolveSlot(slot.b, aQ.gRes, aQ.best8, used3rdAdmin);
                if (slot.a.type === "3rd" && tA && !tA.startsWith("3ro")) used3rdAdmin[tA] = true;
                if (slot.b.type === "3rd" && tB && !tB.startsWith("3ro")) used3rdAdmin[tB] = true;
