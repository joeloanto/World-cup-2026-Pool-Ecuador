...
function PredictView(props) {
  ...
  function buildR32Pairs() {
    // Pre-assign best8 teams to 3rd-place slots optimally
    // Find which group each best8 team belongs to
    var gkeys = Object.keys(GROUPS);
    var teamGroup = {};
    for (var gi = 0; gi < gkeys.length; gi++) {
      var gk = gkeys[gi];
      for (var ti = 0; ti < GROUPS[gk].length; ti++) {
        teamGroup[GROUPS[gk][ti]] = gk;
      }
    }

    // Get all 3rd-place slots and their allowed groups
    var thirdSlots = [];
    for (var si = 0; si < R32_SLOTS.length; si++) {
      var slot = R32_SLOTS[si];
      if (slot.a.type === "3rd") thirdSlots.push({idx: si, side:"a", groups: slot.a.gs.split("/")});
      if (slot.b.type === "3rd") thirdSlots.push({idx: si, side:"b", groups: slot.b.gs.split("/")});
    }

    // Assign best8 teams to slots - each team goes to exactly one slot
    // Use greedy: for each slot, pick the best available team whose group matches
    var slotAssignment = {}; // slot index + side -> team
    var usedTeams = {};

    // Sort slots by number of eligible teams (most constrained first)
    var slotsWithEligible = thirdSlots.map(function(s) {
      var eligible = qualData.best8.filter(function(t) {
        return s.groups.indexOf(teamGroup[t]) >= 0;
      });
      return {slot: s, eligible: eligible};
    });
    slotsWithEligible.sort(function(a, b) { return a.eligible.length - b.eligible.length; });

    for (var xi = 0; xi < slotsWithEligible.length; xi++) {
      var item = slotsWithEligible[xi];
      for (var ei = 0; ei < qualData.best8.length; ei++) {
        var team = qualData.best8[ei];
        if (usedTeams[team]) continue;
        if (item.slot.groups.indexOf(teamGroup[team]) >= 0) {
          slotAssignment[item.slot.idx + "_" + item.slot.side] = team;
          usedTeams[team] = true;
          break;
        }
      }
    ...
    // Build pairs using pre-assigned teams
    var pairs = [];
    for (var pi = 0; pi < R32_SLOTS.length; pi++) {
      var pslot = R32_SLOTS[pi];
      var ta, tb;
      if (pslot.a.type === "3rd") {
        ta = slotAssignment[pi + "_a"] || "Mejor 3ro";
      } else {
        ta = resolveSlot(pslot.a, qualData.gRes, qualData.best8, {});
      }
      if (pslot.b.type === "3rd") {
        tb = slotAssignment[pi + "_b"] || "Mejor 3ro";
      } else {
        tb = resolveSlot(pslot.b, qualData.gRes, qualData.best8, {});
      }
      ...
      {tab === "knockout" && (
        <div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:20}}>
            {(function() {
              // Pre-assign best8 to slots optimally (most constrained first)
              var gkeys2 = Object.keys(GROUPS);
              var teamGrp = {};
              for (var gi2 = 0; gi2 < gkeys2.length; gi2++) {
                var gk2 = gkeys2[gi2];
                for (var ti2 = 0; ti2 < GROUPS[gk2].length; ti2++) teamGrp[GROUPS[gk2][ti2]] = gk2;
              }
              var thirdSlots2 = [];
              ...
              var slotAsgn = {}; var usedT = {};
              thirdSlots2.sort(function(a,b){ return a.groups.length - b.groups.length; });
              for (var xi2 = 0; xi2 < thirdSlots2.length; xi2++) {
                var itm = thirdSlots2[xi2];
                for (var ei2 = 0; ei2 < aQ.best8.length; ei2++) {
                  var tm = aQ.best8[ei2];
                  if (usedT[tm]) continue;
                  if (itm.groups.indexOf(teamGrp[tm]) >= 0) { slotAsgn[itm.idx+"_"+itm.side]=tm; usedT[tm]=true; break; }
                }
              }
              return R32_SLOTS.map(function(slot, i) {
                var tA = slot.a.type==="3rd" ? (slotAsgn[i+"_a"]||"Mejor 3ro") : resolveSlot(slot.a, aQ.gRes, aQ.best8, {});
                var tB = slot.b.type==="3rd" ? (slotAsgn[i+"_b"]||"Mejor 3ro") : resolveSlot(slot.b, aQ.gRes, aQ.best8, {});
